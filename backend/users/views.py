...
class UserViewSet(ModelViewSet):
    ...
    filterset_fields = ['role', 'is_verified_host', 'host_approval_status', 'country', 'city']
...
@extend_schema_view(
    ...
    retrieve=extend_schema(
        summary="Get Host Verification",
        description="Get host verification details"
    ),
    update=extend_schema(
...
class HostVerificationViewSet(ModelViewSet):
    ...
    def verify_document(self, request, pk=None):
        ...
        return api_response(
            success=True,
            data={
                'verification': HostVerificationSerializer(verification).data
            }
        )
...
def host_dashboard_stats(request):
    ...
    if hasattr(user, 'host_verification'):
        ...
        stats['verification_completion'] = verification.verification_completion_percentage()
...
def guest_dashboard_stats(request):
    ...
    stats = {
        ...
        'reviews_written': 0,
        'travel_preferences': user.profile.travel_preferences if hasattr(user, 'profile') else {}
    }
