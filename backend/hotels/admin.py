...
class HotelAdmin(SoftDeleteModelAdmin):
    list_display = ['name', 'city', 'stars', 'rating', 'reviews_count', 'best_seller', 'is_active', 'deleted_status']
    list_filter = ['stars', 'city__country', 'city', 'chain', 'hotel_type', 'best_seller', 'is_active', 'is_deleted']
    ...
    fieldsets = (
        (None, {
            'fields': ('hotel_id', 'name', 'main_photo', 'thumbnail')
        }),
        ('Location', {
        ...
        ('Business Info', {
            'fields': ('chain', 'hotel_type', 'best_seller')
        }),
        ('Facilities', {
            'fields': ('facilities',)
        }),
